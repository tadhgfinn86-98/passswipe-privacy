"""An HttpClient stand-in that serves bundled sample data instead of requests.

Lets the whole pipeline run with no network access, no API keys and no cost.
Used by the tests, by demo.py, and by the web app's Demo mode.
"""
...
class OfflineClient:
    """Routes requests to sample files by matching on the URL."""

    def __init__(self, websites: dict[str, str] | None = None) -> None:
        self.websites = websites if websites is not None else DEMO_WEBSITES
...
def demo_paths() -> dict[str, str]:
    """Paths to the bundled Environment Agency sample registers."""
    return {
        "ea_carriers_csv": str(SAMPLE_DATA / "ea_carriers.csv"),
        "ea_sites_csv": str(SAMPLE_DATA / "ea_sites.csv"),
    }
